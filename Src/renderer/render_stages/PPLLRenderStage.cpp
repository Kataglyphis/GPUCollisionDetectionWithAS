...
PPLLRenderStage::PPLLRenderStage(ResourceManager* rm, DescriptorSet* descriptorSet, RenderAttachmentInfo attachementInfo, uint32_t subpassIndex)
{
    this->rm = rm;
    this->context = rm->context;
    this->descriptorSet = descriptorSet;
    this->subpass = new PPLLRenderStageSubpass(attachementInfo);
    this->subpassIndex = subpassIndex;
}

void PPLLRenderStage::destroyPipeline()
{
    vkDestroyPipelineLayout(context->device, pipelineLayout, nullptr);
    vkDestroyPipeline(context->device, pipeline, nullptr);
}

PPLLRenderStage::~PPLLRenderStage()
{
}

void PPLLRenderStage::compileShaders()
{
    compileShader(context->device, shaders.vert, &shaderModuleVert);
    compileShader(context->device, shaders.frag, &shaderModuleFrag);
}

void PPLLRenderStage::destroyShaders()
{
...
void PPLLRenderStage::createPipeline(VkRenderPass renderPass)
{
    compileShaders();
    ...
    std::vector<VkVertexInputBindingDescription> vertexBindingDescription = { Vertex::getBindingDescription() };
    ...
    pipelineVertexInputStateCreateInfo.vertexBindingDescriptionCount = vertexBindingDescription.size();
    pipelineVertexInputStateCreateInfo.pVertexBindingDescriptions = vertexBindingDescription.data();
...
void PPLLRenderStage::recordRenderCommands(VkCommandBuffer& cmdBuf, VkBuffer* pVertexBuffer, VkBuffer indexBuffer, uint32_t indexCount)
{
    ...
    vkCmdPipelineBarrier(cmdBuf, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 0, nullptr);
    ...
    vkCmdPipelineBarrier(cmdBuf, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 1, &memoryBarrier, 0, nullptr, 0, nullptr);

}

void PPLLRenderStage::recordRenderCommands(VkCommandBuffer& cmdBuf, DrawParams params)
{
    VkDeviceSize offsets[] = { 0 };
    vkCmdBindPipeline(cmdBuf, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline);
    vkCmdBindDescriptorSets(cmdBuf, VK_PIPELINE_BIND_POINT_GRAPHICS,
        pipelineLayout, 0, 1, &descriptorSet->set[context->currentSwapchainIndex], 0, nullptr);
    vkCmdBindVertexBuffers(cmdBuf, 0, 1, &params.geom[1]->vertexBuffer.buffer, offsets);
    vkCmdBindIndexBuffer(cmdBuf, params.geom[1]->indexBuffer.buffer, 0, VK_INDEX_TYPE_UINT32);
    vkCmdDrawIndexed(cmdBuf, params.geom[1]->numIndices, 1, 0, 0, 0);
...
Subpass* PPLLRenderStage::getSubpass()
{
	return subpass;
}
